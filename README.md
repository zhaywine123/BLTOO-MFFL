...
You can directly use BLTOO-MFFL-all-in-one for testing. Please note that if you use the original configuration (without modifying the KL calculation), please do not set a batch_size that is too large. We recommend a maximum of 32, which may result in excessive computation time consumption.
